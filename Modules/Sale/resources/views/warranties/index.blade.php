...
<div class="sale-shell">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3 sale-header">
        <div>
            ...
            <h1 class="page-title display-font mb-1"><i class="bi bi-shield-check"></i> {{ __('sale.warranty_title') }}</h1>
            <p class="page-subtitle mb-0">{{ __('sale.warranty_subtitle') }}</p>
        </div>
        <a href="{{ route('sale.warranties.create') }}" class="btn-new-sale shadow-sm">
    ...
    <div class="filter-toolbar mb-4 shadow-sm">
        <form method="GET" action="{{ route('sale.warranties.index') }}" class="m-0 w-100" id="filterForm">
                ...
                <div class="col-md-3">
                    <select name="status" id="status" class="form-select filter-select-control">
                        ...
                        @foreach(['active', 'claimed', 'expired', 'void'] as $statusOption)
                            <option value="{{ $statusOption }}" {{ ($filters['status'] ?? '') === $statusOption ? 'selected' : '' }}>{{ __('sale.status_' . $statusOption) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                    <button class="btn btn-filter-submit w-100" type="submit">
                        <i class="bi bi-funnel"></i> {{ __('sale.apply_filters') }}
                    </button>
                </div>
            </div>
        </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var table = $('#warrantiesDataTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("sale.warranties.table") }}',
                data: function (d) {
