...
<div class="dashboard-header flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-5 mb-8">
    <div>
        <h1 class="dashboard-title display-font text-3xl font-black text-slate-900 leading-none">{{ __('dashboard.title') }}</h1>
        <p class="dashboard-subtitle text-slate-500 text-sm mt-1.5">{{ __('dashboard.subtitle') }}</p>
    </div>
    ...
    @if(auth()->user()->isOwner() || auth()->user()->isSuperAdmin())
        @if($userShops->count() > 0)
            <div class="dropdown">
                <button class="btn d-flex align-items-center gap-2 px-3 py-1.5 border rounded-pill bg-white shadow-sm dropdown-toggle" 
                    ...
                    <span>{{ $userShops->firstWhere('id', $activeShopId)?->name ?? __('Select Shop Context') }}</span>
                </button>
                <ul class="dropdown-menu dropdown-menu-end border-0 shadow rounded-3 py-1 mt-1" aria-labelledby="shopSelectorDropdown" style="min-width: 220px; border: 1px solid #d8e4ee !important;">
                    ...
                    @foreach($userShops as $s)
                        ...
                        @endphp
                        <li>
                            <form action="{{ route('dashboard.select-shop') }}" method="POST" class="m-0">
                                @csrf
                                ...
                                <button type="submit" 
                                        ...
                                        onmouseover="this.style.background='{{ $isActive ? '#0f766e' : '#f1f5f9' }}'; this.style.color='{{ $isActive ? '#ffffff' : '#0f172a' }}'"
                                        onmouseout="this.style.background='{{ $isActive ? '#0f766e' : 'transparent' }}'; this.style.color='{{ $isActive ? '#ffffff' : '#334155' }}'">
                                    <i class="bi bi-shop {{ $isActive ? 'text-white' : 'text-muted' }}"></i>
                                    <span>{{ $s->name }}</span>
                                </button>
                            </form>
                        </li>
        ...
        @endif
    @elseif(auth()->user()->isManager() && auth()->user()->assignedShop)
        <div class="inline-flex items-center gap-2 bg-slate-100 border border-slate-200 rounded-xl px-4 py-2 text-xs font-bold text-slate-600">
            ...
            <span>Active Shop: {{ auth()->user()->assignedShop->name }}</span>
        </div>
    @endif
</div>
...
<div class="mb-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @foreach($modulesConfig as $modKey => $mod)
            @php
                $hasAccess = auth()->user()->hasModuleAccess($modKey);
            @endphp
            <div class="bg-white border border-slate-200/80 rounded-2xl p-5 flex flex-col h-full shadow-sm hover:shadow-md transition-all duration-200 relative group {{ !$hasAccess ? 'border-dashed' : '' }}">
                @if(!$hasAccess)
                ...
                <div class="flex items-start justify-between gap-4 mb-4 z-20">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center border shadow-inner {{ $hasAccess ? 'bg-teal-50 border-teal-100 text-teal-600' : 'bg-slate-100 border-slate-200 text-slate-400' }}">
                        <i class="bi {{ $mod['icon'] }} text-lg"></i>
                    </div>
                    <div>
                        @if($hasAccess)
                ...
                <div class="flex-grow z-20">
                    <h3 class="font-bold text-slate-900 text-sm mb-1 group-hover:text-teal-600 transition-colors">{{ $mod['name'] }}</h3>
                    <p class="text-slate-500 text-xs leading-relaxed">{{ $mod['desc'] }}</p>
                </div>
